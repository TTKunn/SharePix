...
void LikeHandler::registerRoutes(httplib::Server& server) {
    ...
    // 查询点赞状态
    server.Get("/api/v1/posts/:post_id/like/status", [this](const httplib::Request& req, httplib::Response& res) {
        handleGetLikeStatus(req, res);
    });
...
// 处理查询点赞状态请求
void LikeHandler::handleGetLikeStatus(const httplib::Request& req, httplib::Response& res) {
    try {
        // 1. 验证JWT令牌并获取用户ID
        int userId = 0;
        if (!authenticateRequest(req, userId)) {
        ...
        Logger::error("Exception in handleGetLikeStatus: " + std::string(e.what()));
        sendJsonResponse(res, 500, false, "服务器内部错误");
    }
}
