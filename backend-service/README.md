...
- **[🏗️ 架构文档](project_document/[001]项目架构文档.md)** - 九层架构设计和技术栈
- **[⚙️ 环境配置](project_document/[002]环境安装配置教程.md)** - 开发环境搭建指南
...
2. **[项目架构文档](project_document/[001]项目架构文档.md)** - 了解整体架构
3. **[环境安装配置教程](project_document/[002]环境安装配置教程.md)** - 搭建开发环境
4. **[API文档](project_document/[000]API文档.md)** - 了解所有API接口
5. **[数据库设计文档](project_document/[200]数据库设计文档.md)** - 了解数据库结构
6. **阶段文档** - 按需阅读各阶段的实现细节

#### 按角色查看

- **前端开发者**: [API文档](project_document/[000]API文档.md) → [项目架构文档](project_document/[001]项目架构文档.md)
- **后端开发者**: [文档总览](project_document/README.md) → [项目架构文档](project_document/[001]项目架构文档.md) → [数据库设计文档](project_document/[200]数据库设计文档.md) → 阶段文档
- **运维工程师**: [环境安装配置教程](project_document/[002]环境安装配置教程.md) → [性能优化专题](project_document/[201]性能优化专题.md)
- **项目经理**: [文档总览](project_document/README.md) → [项目架构文档](project_document/[001]项目架构文档.md)
...
│   │   └── parking_space_handler.h/cpp # 停车位接口处理
│   ├── core/                  # 核心业务逻辑层
│   │   ├── auth_service.h/cpp # 认证服务
│   │   ├── user_service.h/cpp # 用户服务
│   │   └── parking_space_service.h/cpp # 停车位服务
│   ├── database/              # 数据访问层
│   │   ├── connection_pool.h/cpp # 数据库连接池
│   │   ├── connection_guard.h/cpp # RAII连接管理
│   │   ├── user_repository.h/cpp # 用户数据访问
│   │   └── parking_space_repository.h/cpp # 停车位数据访问
│   ├── models/                # 数据模型层
│   │   ├── user.h/cpp         # 用户模型
│   │   └── parking_space.h/cpp # 停车位模型
│   ├── security/              # 安全层
...
2. **创建数据库和用户**:
   ...
   CREATE USER 'parking_user'@'localhost' IDENTIFIED BY 'secure_password';
...
3. **初始化数据库表**:
   ```bash
   mysql -u parking_user -p shared_parking < config/database.sql
   ```
...
### 5. 运行服务
```bash
./shared_parking_auth ../config/config.json
```
...
| 接口 | 方法 | 路径 | 说明 |
|------|------|------|------|
| 添加停车位 | POST | /api/v1/my-spaces | 添加新停车位 |
| 查看停车位列表 | GET | /api/v1/my-spaces | 查看我的停车位列表 |
| 查看停车位详情 | GET | /api/v1/my-spaces/:space_id | 查看停车位详情 |
| 编辑停车位 | PUT | /api/v1/my-spaces/:space_id | 编辑停车位信息 |
| 删除停车位 | DELETE | /api/v1/my-spaces/:space_id | 删除停车位 |
...
#### 添加停车位
```bash
curl -X POST http://localhost:8080/api/v1/my-spaces \
...
- **[001]项目架构文档.md**: 架构设计和技术栈
- **[002]环境安装配置教程.md**: 环境搭建指南
- **[100]阶段A-基础模块实现.md**: 基础模块实现
- **[101]阶段B-用户认证模块.md**: 用户认证系统
- **[102]阶段C-停车位管理模块.md**: 停车位管理系统
